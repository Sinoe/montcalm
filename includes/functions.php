...
//$baseRedirectURL = 'https://www.restaurant-montcalm.fr/';
$baseRedirectURL = 'http://localhost/dev/montcalm/';
...
/* MONTCALM */	
	/*PLAT FRONT*/
		...
		function getTodayCommandes(){
			$mysqli = $this->connect;
			$sql = "SELECT * FROM commandes WHERE DATE(`date`) = CURDATE() ORDER BY `id` DESC" ;
			$query = mysqli_query($this->connect, $sql) or die(mysqli_error($this->connect));
	...
	/*MAILS*/
		/*MAILS*/
		public function sendMail($message, $email, $id, $type){
		...
		// return mail($to, $subject, $message, $headers);
			...
			if($type == 'boys'){
				$mail->setFrom('commande@restaurant-montcalm.fr', 'Restaurant Montcalm');
				$mail->addAddress($to);     // Add a recipient
				$mail->addAddress('sinoe.grattepanche@gmail.com'); 
				$mail->Subject = "✔️🧈🍲 Nouvelle commande du site => #".$id;
			} else {
				$mail->setFrom('ne-pas-repondre@restaurant-montcalm.fr', 'Restaurant Montcalm');
				$mail->addAddress($to);     // Add a recipient
				//$mail->addAddress('sinoe.grattepanche@gmail.com'); 
				$mail->Subject = "✔️ Validation de votre commande - Restaurant Montcalm (#".$id.')';
			}
			...
			<!-- HEADER -->
				<tr class="psingle">
				    <td class="one-column">
				        <table width="100%">
				            <tbody><tr>
				                <td align="center"><a href="https://www.retaurant-montcalm.fr"><img width="200" style="width:200px; margin:0 auto;" src="https://sinoegrattepanche.com/img/logo-montcalm.png" alt="Logo Montcalm" /></a></td>
				            </tr>
				           	</tbody>
				        </table>
				    </td>
